...
export type X11EventData = {     os_info: OsInfo; desktop_names: string []; current_desktop_id: number; focused_window: number; ms_since_user_input: number;     ms_until_screensaver: number; screensaver_window: number; network:     NetworkInfo | null; windows: X11WindowData [] };
export type X11WindowData = {     window_id: number; geometry: X11WindowGeometry; process: ProcessData     | null; window_properties: { [key: string]: J } };
export type X11WindowGeometry = { x: number; y: number; width: number; height: number };
export type ProcessData = {     pid: number; name: string; cmd: string []; exe: string; cwd:     string; memory_kB: number; parent: number | null; status: string;     start_time: DateTime<Utc>; cpu_usage: number | null };
export type NetworkInfo = { wifi: WifiInterface | null };
export type WifiInterface = {     ssid: string; mac: string; name: string; power: number;     average_signal: number; bssid: string; connected_time: number };
export type OsInfo = {     os_type: string; version: string; batteries: number | null; hostname: string; username: string | null; machine_id: string | null };
export type TagRuleGroup = { global_id: string; data: Json<TagRuleGroupData>};
export type TagRuleGroupData = 
 | { version: "V1"; data: TagRuleGroupV1 };
...
export type TagRuleGroupV1 = {     name: string; description: string; editable: boolean; enabled:     boolean; rules: TagRuleWithMeta [] };
...
export type ApiTypesTS = 
 ...
 | {     type: "timestamp_search"; request: (TimestampSearchRequest); response: (Timestamptz | null) };
export type SingleExtractedEventWithRaw = {     id: string; timestamp_unix_ms: Timestamptz; duration_ms: number;     tags: Tags; raw: EventData | null; tags_reasons:     { [key: string]: TagAddReason } | null };
export type ApiResponse<T> = { data: T };
export type SingleExtractedChunk = {     from: Timestamptz; to_exclusive: Timestamptz; tags:     [string, string, number] [] };
